...
import { FiDribbble, FiFacebook, FiInstagram, FiSend, FiTwitter } from 'react-icons/fi'
...
export function Footer() {
  const socialLinks = [
    ...
    {id:4, icon:<FiDribbble size={28} />, linkLabel:'Dribbble.com/DigitalUx'},
  ]
  const navLinks = [
    ...
    <footer className='max-w-[1440px] w-full mx-auto mt-40 mb-5 px-4 flex items-start justify-between gap-4'>
      ...
      <div className='hidden lg:block'>
        ...
        <ul className='mt-6 flex flex-col gap-4'>
          {navLinks.map(({id, linkUrl, linkLabel}) => (
            <li key={id}>
              <Link href={linkUrl} className='hover:font-semibold hover:border-b-2 ease-in-out duration-150'>
                {linkLabel}
              </Link>
            </li>
          ))}
      ...
      <div className='hidden lg:block'>
        ...
        <ul className='mt-6 flex flex-col gap-4'>
          {socialLinks.map(({id, icon, linkLabel}) => (
            <li key={id} className='flex items-center gap-2'>{icon}{linkLabel}</li>
          ))}
        </ul>
      </div>
      ...
      <div className='max-w-[250px]'>
        ...
        <label className='relative'>
          <input type="email" placeholder='Your email' className='w-full mt-8 py-2 px-5 text-black rounded border border-[#767676]' />
